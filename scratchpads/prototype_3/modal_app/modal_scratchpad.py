...
def label_clusters(username: str, force_recompute: bool = False):
    ...
    if has_level0_labels:
        ...
        # handle 100,000 token limit per minute by counting chars at 0.5 token per char and batching accordingly
        batches = []
        max_chars = 100000 / 0.5
        current_batch = []
        current_chars = 0
        for cluster_id, text in cluster_id_text_tuples:
            text_length = len(text) + len(LABEL_CLUSTER_PROMPT)
            if current_chars + text_length > max_chars:
                batches.append(current_batch)
                current_batch = []
                current_chars = 0
            current_batch.append((cluster_id, text))
        ...
        for i, batch in enumerate(batches):
            level0_results.extend(label_one_cluster.starmap(batch, order_outputs=True))
            if i < len(batches) - 1:
                time.sleep(60)
